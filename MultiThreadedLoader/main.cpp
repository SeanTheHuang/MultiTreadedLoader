	...
	case WM_COMMAND:
	{
		...
		case ID_FILE_LOADIMAGE:
		{
			if (ChooseImageFilesToLoad(_hwnd))
			{
				...
				int iImagesPerSection = (int)ceilf(sqrtf((float)iMaxImages));

				int iImageWidth = (g_ScreenRect.right - g_ScreenRect.left) / iImagesPerSection;
				int iImageHeight = (g_ScreenRect.bottom - g_ScreenRect.top) / iImagesPerSection;

				int iStepX = 0, iStepY = 0;	//Use for tiling images in screen
				...
				while (!g_vecImageFileNames.empty())	//Assign image drawing to threads
				{
					...
					if (iStepX >= iImagesPerSection)	//Start drawing next row
					{
						iStepY++;
						iStepX = 0;
					}
			...
			{
				MessageBox(_hwnd, L"No Image File selected or Too Many Files selected", L"Error Message", MB_ICONWARNING);
			}
		...
		case ID_FILE_LOADSOUND:
		{
			...
			{
				MessageBox(_hwnd, L"No Sound File selected or Too Many Files selected", L"Error Message", MB_ICONWARNING);
			}
			return (0);
		}
		break;
	...
	int _nCmdShow)
{
	...
	GetClientRect(_hwnd, & g_ScreenRect);
	g_backBuffer.Initialise(_hwnd, g_ScreenRect.right, g_ScreenRect.bottom);
